"""
Collect 2024 and 2025 Draft Class QB Data using CFBD API

Uses CFBD API to collect recent QB data that's not yet in sportsdataverse
"""
...
def main():
    """Collect 2024/2025 QB data using CFBD"""
    print("=" * 80)
    print("COLLECTING 2024/2025 QB DATA USING CFBD API")
    print("=" * 80)
    ...
    # Load drafted QBs list
    print("\n[2] Loading 2024/2025 QB list...")
    drafted = pd.read_csv(root / 'data' / 'raw' / 'drafted_qbs.csv')
    recent_qbs = drafted[drafted['draft_year'] >= 2024].copy()

    print(f"    [OK] Found {len(recent_qbs)} QBs from 2024/2025")
    ...
    qbs_to_collect = []
    for _, qb in recent_qbs.iterrows():
        if qb['player_name'] not in existing:
            qbs_to_collect.append(qb)

    print(f"\n[3] Need to collect {len(qbs_to_collect)} QBs")

    if len(qbs_to_collect) == 0:
        print("    [OK] All QBs already collected!")
        return
