"""
WR Composite Rating Game-by-Game Visualizer

Interactive dashboard for viewing WR composite ratings with:
...
Usage:
    python visualize_wr_ratings.py

    Then open browser to: http://localhost:8051
"""
...
def load_wr_data():
    """Load WR game-by-game composite ratings data."""
    logger.info("Loading WR game-by-game composite ratings...")

    # Load game-by-game data
    wr_files = sorted(DATA_DIR.glob("wr_composite_game_by_game_*.csv"))
    if not wr_files:
        raise FileNotFoundError("No WR game-by-game file found in data/production")

    wr_data = pd.read_csv(wr_files[-1])
    logger.info(f"  Loaded {len(wr_data):,} game records from {wr_data['receiver_player_id'].nunique()} WRs")

    return wr_data

def filter_to_wrs(wr_data, min_games=10):
    """Filter to WRs with minimum games for visualization."""
    game_counts = wr_data.groupby('receiver_player_id').size()
    qualified_wrs = game_counts[game_counts >= min_games].index

    wr_data_filtered = wr_data[wr_data['receiver_player_id'].isin(qualified_wrs)].copy()

    # Get most recent rating for each WR
    wr_data_sorted = wr_data_filtered.sort_values(['receiver_player_id', 'season', 'week'])
    most_recent = wr_data_sorted.groupby('receiver_player_id').tail(1).copy()
    most_recent = most_recent.sort_values('smoothed_composite', ascending=False)

    logger.info(f"  Filtered to {len(most_recent)} WRs with {min_games}+ games")

    return wr_data_filtered, most_recent
...
def calculate_predictions(player_games, years_forward=[1, 2, 3]):
    ...
    # WR Career Trajectory (from analyze_career_trajectories.py):
    #   Games 0-32: Improve +0.13/season (R²=0.907)
    #   Games 32-80: Improve +0.14/season (R²=0.669)
    #   Peak at Games 80-87 (rating 0.49)
    #   Games 87+: Slight decline -0.08/season (80% of players decline)
    ...
    for years in years_forward:
        ...
        if total_games < 32:
            # Early career - strong improvement phase
            career_adjustment = 0.13 * years
        elif total_games < 80:
            # Mid career - continued improvement toward peak
            career_adjustment = 0.14 * years
        elif total_games < 87:
            # Peak years - maintain with slight improvement
            career_adjustment = 0.05 * years
        else:
            # Post-peak - gradual decline
            career_adjustment = -0.08 * years
        ...
        if total_games < 32 and future_games > 32:
            ...
            career_adjustment = (games_in_early / 16) * 0.13 + (games_in_mid / 16) * 0.14
        elif total_games < 80 and future_games > 80:
            games_in_mid = 80 - total_games
            games_in_peak = future_games - 80
            career_adjustment = (games_in_mid / 16) * 0.14 + (games_in_peak / 16) * 0.05
        elif total_games < 87 and future_games > 87:
            games_in_peak = 87 - total_games
            games_in_decline = future_games - 87
            career_adjustment = (games_in_peak / 16) * 0.05 + (games_in_decline / 16) * (-0.08)
        ...
        # Add regression to mean (elite WRs tend to decline, poor WRs improve)
        ...
        mean_rating = 0.0  # Average WR is 0.0 z-score
...
def create_player_trajectory_figure(player_games, predictions, player_name):
    ...
    fig = make_subplots(
        rows=1, cols=1,
        subplot_titles=[f"{player_name} - WR Composite Rating Trajectory"]
    )
    ...
    # Add reference line at 0 (average WR)
    fig.add_hline(y=0, line_dash="dot", line_color="gray",
                  annotation_text="Average WR (0.0)", annotation_position="right")
    ...
    fig.update_layout(
        ...
        yaxis_title="WR Composite Rating (Z-Score)",
...
app = dash.Dash(
    ...
    title="WR Composite Rating Visualizer"
)

# Load data
wr_data = load_wr_data()
wr_data_filtered, wr_summary = filter_to_wrs(wr_data, min_games=10)

# App layout
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H1("🎯 WR Composite Rating Visualizer", className="text-center mb-4"),
            html.P(
                "Interactive game-by-game visualization of WR composite ratings with predictions",
    ...
    dbc.Row([
        dbc.Col([
            html.Label("Select WR:", className="fw-bold"),
            dcc.Dropdown(
                id='player-dropdown',
                options=[
                    {'label': f"{row['receiver_player_name']} ({row['smoothed_composite']:.3f})",
                     'value': row['receiver_player_id']}
                    for _, row in wr_summary.iterrows()
                ],
                value=wr_summary.iloc[0]['receiver_player_id'],  # Default to top WR
                clearable=False,
                className="mb-3"
            )
        ], width=6),
...
def update_visualizations(player_id, prediction_years):
    ...
    # Get player data
    player_games = wr_data_filtered[wr_data_filtered['receiver_player_id'] == player_id].copy()
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("WR Composite Rating Visualizer")
    logger.info("=" * 80)
    logger.info(f"\nLoaded {len(wr_summary)} WRs with 10+ games")
    logger.info(f"Total game records: {len(wr_data):,}")
    logger.info("\nStarting web server...")
    logger.info("Open browser to: http://localhost:8051")
