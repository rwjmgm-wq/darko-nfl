"""
RB Composite Analysis - Phase 2: Exhaustive Combination Search

Tests all possible 5-metric combinations from the top 15 candidates
to find the optimal RB performance composite.
"""
...
def main():
    """Run Phase 2: Exhaustive combination search for RBs."""

    logger.info("="*80)
    logger.info("RB COMPOSITE ANALYSIS - PHASE 2: EXHAUSTIVE SEARCH")
    logger.info("="*80)
    ...
    candidates_file = Path("data/processed/rb_metric_candidates.csv")
    if not candidates_file.exists():
        raise FileNotFoundError("Phase 1 results not found. Run rb_analyze_metric_candidates.py first.")
    ...
    # Load RB Data
    # ==================================================
    logger.info("\n[2/3] Loading RB data for 2023-2024...")
    ...
    logger.info(f"  2023: {len(season_stats_2023)} RBs")
    logger.info(f"  2024: {len(season_stats_2024)} RBs")
    ...
    # Save results
    output_file = Path("data/processed/rb_exhaustive_combinations.csv")
    ...
    # Display top 10
    print("\n" + "="*80)
    print("TOP 10 RB COMPOSITE COMBINATIONS")
    print("="*80)
    ...
    logger.info(f"Next step: Run rb_multi_year_validation.py (Phase 3)")
