"""
QB Composite Analysis - Phase 4: Multi-Year Validation

Tests top QB composite combinations across multiple year pairs (2020-2024)
...
def main():
    """Run Phase 4: Multi-year validation for QBs."""

    logger.info("="*80)
    logger.info("QB COMPOSITE ANALYSIS - PHASE 4: MULTI-YEAR VALIDATION")
    logger.info("="*80)
    ...
    combos_file = Path("data/processed/qb_exhaustive_combinations.csv")
    if not combos_file.exists():
        raise FileNotFoundError("Phase 2 results not found. Run qb_exhaustive_composite_search.py first.")
    ...
    for year in range(2020, 2025):
        ...
        ratings = aggregator.aggregate_season_stats(game_stats, min_attempts=100)
        ratings_by_year[year] = ratings
        logger.info(f"    {year}: {len(ratings)} QBs")
    ...
    # Save results
    output_file = Path("data/processed/qb_multi_year_validation.csv")
    ...
    # Display results
    print("\n" + "="*80)
    print("QB MULTI-YEAR VALIDATION RESULTS")
    print("="*80)
    ...
    print(f"\nRecommended QB Composite Rating (most stable):")
    ...
    # Compare to WRs
    print("\n  Comparison to WR analysis:")
    print(f"    QB multi-year r: {best_combo['r_mean']:.4f}")
    print(f"    WR multi-year r: 0.6614 (from WR analysis)")
    print(f"    QBs are {(best_combo['r_mean']/0.6614 - 1)*100:+.1f}% more stable than WRs")

    logger.info("\n" + "="*80)
    logger.info("PHASE 4 COMPLETE!")
    logger.info("="*80)
    logger.info(f"\nMulti-year validation confirms QB composite stability.")
    logger.info(f"Ready to implement QB composite rating system.")
