"""
EDGE Rusher Exhaustive Composite Metric Search

Tests all possible 3-5 metric combinations for EDGE rushers (DL/LB) to find which
...
# EDGE RUSHER CANDIDATE METRICS
# Focus on pass rush, run defense, and playmaking ability
EDGE_METRICS = [
...
def load_edge_data(years=range(2020, 2025)):
    """Load EDGE rusher defensive data."""

    logger.info("="*80)
    logger.info("LOADING EDGE RUSHER DATA")
    logger.info("="*80)
    ...
    for year in years:
        ...
        all_edge_stats.append(season_stats)

    edge_data = pd.concat(all_edge_stats, ignore_index=True)
    logger.info(f"\nLoaded {len(edge_data)} EDGE-seasons")
    logger.info(f"Unique EDGE rushers: {edge_data['player_id'].nunique()}")

    return edge_data


def test_metric_combination(df, metrics, target_col='next_season_pressures_per_game'):
...
def exhaustive_search(df, target_col, min_metrics=3, max_metrics=5):
    ...
    available_metrics = [m for m in EDGE_METRICS if m in df.columns]
...
def main():
    # Load data
    edge_data = load_edge_data(years=range(2020, 2025))

    logger.info(f"\n{'='*80}")
    logger.info("DATA SUMMARY")
    logger.info(f"{'='*80}")
    logger.info(f"Total EDGE-seasons: {len(edge_data)}")
    logger.info(f"Unique players: {edge_data['player_id'].nunique()}")
    logger.info(f"Seasons: {sorted(edge_data['season'].unique())}")
    logger.info(f"\nPosition breakdown:")
    logger.info(edge_data['position'].value_counts().to_string())

    # Create next season target variables
    edge_data = edge_data.sort_values(['player_id', 'season'])

    # Test multiple target variables to find best predictor
    target_variables = {
        'next_season_pressures_per_game': 'pressures_per_game',
        'next_season_sacks_per_game': 'sacks_per_game',
    ...
    for target_name, source_col in target_variables.items():
        edge_data[target_name] = edge_data.groupby('player_id')[source_col].shift(-1)

    # Filter to valid samples (players with next season data)
    edge_data_filtered = edge_data[edge_data['next_season_pressures_per_game'].notna()].copy()

    logger.info(f"\nValid samples (with next season data): {len(edge_data_filtered)}")
    logger.info(f"Unique players: {edge_data_filtered['player_id'].nunique()}")
    ...
    # Save results
    output_file = "data/processed/edge_exhaustive_search_results.csv"
    ...
    logger.info(f"\n{'='*80}")
    logger.info("RECOMMENDED COMPOSITE FOR EDGE RUSHERS")
    logger.info(f"{'='*80}")
