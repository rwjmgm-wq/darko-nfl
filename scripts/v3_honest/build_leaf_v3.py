"""
LEAF v3 (QB DARKO) - walk-forward rating engine. Implements docs/SPEC_LEAF_V3.md.
...
def load_games():
    # Prefer the fresh full-2025 base file (refresh_game_data.py); fall back to
    # the legacy leaf_v2 export (frozen Nov 2025).
    fresh = ROOT / 'data' / 'production' / 'qb_games_base.csv'
    legacy = ROOT / 'data' / 'production' / 'leaf_v2_game_by_game_20251111.csv'
    df = pd.read_csv(fresh if fresh.exists() else legacy)
...
def fit_age_drift(df):
    ...
    for pid, g in df[df['season'] <= TRAIN_MAX_SEASON].groupby('passer_player_id'):
        ...
        # pandas >= 2.2; nfl_data_py pins 1.5.3 in CI, and the explorer's
        # pipeline/leaf_engine.py mirror must stay runnable there)
